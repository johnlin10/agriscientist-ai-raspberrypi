...
# print(data_list[-1])  # 打印列表中的第一個元素
# print(data_list[-1]["temperature"])  # 打印列表中的第一個元素
# print(data_list[-1]["humidity"])  # 打印列表中的第一個元素
...
# 聊天記錄設定
KEEP_RECENT = 11  # 設置要保留的預訓練 Prompt
...
def chat(content):
    ...
    sensor_data_list = sensor_data["data"]  # 假設 'data_list_key' 是包含數據列表的鍵

    # 數據 Prompt
    data_prompt = {
        "role": "system",
        "content": f"以下為目前蒐集到的最新農場數據：搜集時間：{sensor_data_list[-1]['timestamp']}，空氣溫度：{sensor_data_list[-1]['temperature']}，空氣濕度：{sensor_data_list[-1]['humidity']}",
    }
